...
class rtpproxy_widget extends widget
{
  public $active = 0;
  public $inactive = 0;
  public $probing = 0;
  ...
  public static function fetch_box_sets($params) {
    ...
    if ($mi_box == null)
      return array();
    require_once("../../../common/mi_comm.php");
    $parititons = mi_command("rtpproxy_show", null, $mi_box['mi_conn'], $errors);
    if (count($errors) != 0) {
      error_log(print_r($errors, true));
      return array();
    }
  ...
  public static function rtp_box_selection($set) {
        echo ('
            ...
            function update_sets() {
                ...
                fetch_widget_info("rtpproxy_widget", "fetch_box_sets", "widget_box="+selected_box)
                .then(data => {
		              data.forEach(set => {
